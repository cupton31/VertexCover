...
public class Graph {
   ...
   /**
    ...
    /**
     * Return the number of live vertices in the graph
     */
    public int v() { return v; }
   ...
   /**
    ...
    public void removeVertex (int v) {
    	for (int i=0; i<V; i++) { // For all vertecies.
    		if (i==v) {
    			E -= adj[i].size();
    			//adj[i]=null; // Remove v.
    			// Remove all edges (items) from adj[i]
    			int puu = adj[i].size();
    			for (int ii=0; ii<puu; ii++) {
    				adj[i].removeFirst();
    			}
    			adj[i].live=false;
    			this.v--;
    		} else adj[i].remove(v); // Else remove edge from i to v.
